...
const debug = debugFactory('taube-subscriber')
...
class Subscriber {
  ...
  async setupChannel() {
    ...
    if (!this.channel) {
      this.channel = await amqp.channel(this.options)
        ...
        this.consumer = await channel.consume(this.q.queue, async(msg) => {
          ...
          if (msg == null) {
            ...
            return Object.keys(this.fns)
              .forEach((topic) => this.fns[topic]
                .forEach((fn) => this.on(topic, fn)
                  .then().catch(amqp.getErrorHandler())))
          }
