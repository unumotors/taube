...
class Server {
  ...
  get(path, validate, fn) {
    ...
    this.router.get(uriHelper.fixServerPath(path), celebrate(validate), async(req, res) => {
  ...
  post(path, validate, fn) {
    checkParameters(path, validate, fn)
    this.router.post(uriHelper.fixServerPath(path), celebrate(validate), async(req, res) => {
      await executeAndReturnResult(req, res, fn)
    })
    ensureErrorHandlingMiddlewareIsLast()
  }
  ...
  put(path, validate, fn) {
    checkParameters(path, validate, fn)
    this.router.put(uriHelper.fixServerPath(path), celebrate(validate), async(req, res) => {
      await executeAndReturnResult(req, res, fn)
    })
    ensureErrorHandlingMiddlewareIsLast()
  }
  ...
  delete(path, validate, fn) {
    checkParameters(path, validate, fn)
    this.router.delete(uriHelper.fixServerPath(path), celebrate(validate), async(req, res) => {
      await executeAndReturnResult(req, res, fn)
    })
    ensureErrorHandlingMiddlewareIsLast()
  }
