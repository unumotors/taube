...
const debug = debugFactory('taube-queue-v2')
...
class QueueWorkerExponentialRetries {
  ...
  async retryMessage({
    ...
    // The routing key will have QUEUE_NAME.RETRY{SECONDS}
    // e.g. some-queue-name.imei.RETRY1000 or some-retry-name.RETRY5000
    // (the integer at the end is the duration of the retry)
    // This does remove the RETRY part
    const keyRoutingSegments = message.fields.routingKey.split('.')
    if (keyRoutingSegments[keyRoutingSegments.length - 1].includes('RETRY')) {
    ...
    if (attemptNumber < this.delays.length) {
      ...
      keyRoutingSegments.push(`RETRY${delay}`)
